...
def header(stdt, etdt, divisioncode):
    c.setTitle('DGGainLossLot-DateWise')
    fonts(15)
    c.setFillColorRGB(0, 0, 0)
    c.drawCentredString(300, 800, divisioncode[-1])
    fonts(9)
    # c.drawString(10, Yaxis, str((date.today()).strftime('%d/%m/%y')))
    c.drawCentredString(300, 785, "D. G. Gain/Loss Report (Lot-DateWise) From  " + str(stdt.strftime(' %d  %B  %Y'))
    ...
    c.drawString(130, 765, 'Adj. No. ')
    c.drawString(250, 765, 'Date ')
...
def data(result, d, stdt, etdt, divisioncode):
    fonts(7)
    c.drawString(31, d, str(result['LOTNO']))
    c.drawString(131, d, str(result['ADJNO']))
    c.drawString(251, d, str(result['DATE'].strftime('%d-%m-%Y')))

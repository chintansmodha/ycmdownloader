...
def header(stdt,etdt,d,result,plantcode):
    ...
    fonts(9)
    c.drawCentredString(300, 780,"Packing Material Ledger Summary (Customer) From " + str(stdt.strftime('%d-%m-%Y')) + " To " + str(etdt.strftime('%d-%m-%Y')))
    fonts(9)
    c.line(0, 760, 1000, 760)
    c.line(0, 740, 1000, 740)
    c.drawString(40, 745, "Customer")
...
def data(stdt,etdt,result,d):
    fonts(7)
    c.drawString(40, d, str(result['CUSTOMER']))
    c.drawAlignedString(365, d, str(int(float(result['OPBAL']))))
    c.drawAlignedString(425, d, str(int(float(result['RECEIVEDQUANTITY']))))
    c.drawAlignedString(485, d, str(int(float(result['ISSUEDQUANTITY']))))
...
def Pallettotal(result):
    ...
    opQty = opQty + float(result['OPBAL'])
    rQty = rQty + float(result['RECEIVEDQUANTITY'])
    IssueQty = IssueQty + float(result['ISSUEDQUANTITY'])
    PalletBalance = PalletBalance + float(result['BALANCE'])
    DepopQty = DepopQty + float(result['OPBAL'])
    DeprQty = DeprQty + float(result['RECEIVEDQUANTITY'])
    DepIssueQty = DepIssueQty + float(result['ISSUEDQUANTITY'])

...
def header(stdt,etdt,transporter,despatch):
    boldfonts(8)
    c.drawCentredString(310, 810, "Transporter+Party wise despatch summary From " + stdt.strftime('%d-%m-%Y') + " To " + etdt.strftime('%d-%m-%Y'))
    p=page()
    c.drawString(500,800,"Page No."+str(p))
    c.line(0, 790, 650, 790)
    c.line(0, 770, 650, 770)
    c.drawString(10, 778, "Company / Party")
    c.drawString(415, 778, "Quantity")
    c.drawString(475, 778, "Freight")
    c.drawString(535, 778, "Service Tax")

def data(result,d):
    fonts(8)
    c.drawString(20, d, str(result['PARTY']))
    if result['QUANTITY']!=None:
        c.drawAlignedString(435, d, str(result['QUANTITY']))
    if result['FREIGHT']!=None:
        c.drawAlignedString(495, d, str(result['FREIGHT']))
    partytotal(result)
    brokertotal(result)
    despatchTotal(result)
...
def partytotalprint(d,stdt,etdt):
    ...
    c.drawAlignedString(435,(d),str(round(TotalPartyQty,3)))
    c.drawAlignedString(495,(d),str(round(TotalPartyAmt,2)))
    TotalPartyQty=0
    TotalPartyAmt=0

def despatchTotal(result):
    global TotalDespatchQty
    global TotalDespatchAmt
    if result['QUANTITY']!=None:
        TotalDespatchQty=TotalDespatchQty+(float("%.2f"%float(result['QUANTITY'])))
    if result['FREIGHT']!=None:
        TotalDespatchAmt=TotalDespatchAmt+(float("%.2f"%float(result['FREIGHT'])))

def printDespatchTotal(d,stdt,etdt):
    global TotalDespatchQty
    global TotalDespatchAmt

    c.drawString(10,d,"Transporter Total :")
    c.drawAlignedString(435,(d),str(round(TotalDespatchQty,3)))
    c.drawAlignedString(495,(d),str(round(TotalDespatchAmt,2)))
    TotalDespatchQty=0
    TotalDespatchAmt=0
...
def logic(result):
    despatch.append(result['COMPANY'])
...
def textsize(c,result, d, stdt, etdt):
    ...
    if len(transporter) == 1:
        if len(despatch) == 1:
            header(stdt,etdt,transporter,despatch)
            c.drawCentredString(310,d,transporter[-1])
            d = dvalue(stdt, etdt, transporter)
        ...
        partytotalprint(d,stdt,etdt)
        d= dvalue(stdt, etdt, transporter)
        printDespatchTotal(d,stdt,etdt)   
        d= dvalue(stdt, etdt, transporter)
        d= dvalue(stdt, etdt, transporter)
        c.drawCentredString(310,d,transporter[-1])
        d= dvalue(stdt, etdt, transporter)

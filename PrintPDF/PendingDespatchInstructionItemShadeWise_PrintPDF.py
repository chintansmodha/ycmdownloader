...
def data(result, d):
    ...
    c.drawAlignedString(540, d, str(format_number(float(result['PENDINGQTY']), locale='en_IN')))
    ...
    if len(divisioncode) == 1:
        ...
        if Item[-1] == Item[-2]:
            if Shade[-1] != Shade[-2]:
                ...
                c.drawAlignedString(540, d, str(format_number(float(ShadeBalQty_Total), locale='en_IN')))
                ShadeTotalClean()
                d = dvalue()
                c.drawString(60, d, Shade[-1])
                d = dvalue()
        ...
        c.drawAlignedString(540, d, str(format_number(float(GrandBal_Total), locale='en_IN')))
        ShadeTotalClean()
        ItemTotalClean()
        GrandTotalClean()

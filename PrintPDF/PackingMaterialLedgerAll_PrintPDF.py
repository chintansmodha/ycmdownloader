...
def dvalue(stdt, etdt,result, plantcode):
    ...
    if d > 20:
        ...
        c.showPage()
        d = newpage()
        header(stdt, etdt,result,d ,plantcode)
        # d = dvalue(stdt, etdt,result, plantcode)
        return d

def dvalueAdd(stdt, etdt,result, plantcode):
    global d
    if d > 20:
        d = d + 10
        return d
...
def header(stdt,etdt,d,result,plantcode):
    ...
    # c.drawString(70, 730, str(supplier[-1]))
    # c.drawString(300, 730, str("PALLETE TYPE:"))
    # f = 730
    # if len(itemtype[-1]) > 45:
    #
    #     lines = textwrap.wrap(itemtype[-1], 45, break_long_words=False)
    #     for i in lines:
    #         c.drawString(370, f, str(i))
    #         f = f - 10
    # else:
    #     c.drawString(370, f, itemtype[-1])
    #     f = f - 10
...
def data(stdt,etdt,result,d):
    ...
    # c.drawAlignedString(550, d, str(("%.0f" % float(result['BALANCE']))))
    total(result)
    return d
...
def textsize(c, result, d, stdt, etdt):
    ...
    if len(plantcode) == 1:
        ...
        if len(str(itemtype[-1])) > 40:

            lines = textwrap.wrap(str(itemtype[-1]), 40, break_long_words=False)
            for i in lines:
                c.drawString(370, d, str(i))
                d = dvalue(stdt, etdt, result, plantcode)
            d = dvalueAdd(stdt, etdt, result, plantcode)
        else:

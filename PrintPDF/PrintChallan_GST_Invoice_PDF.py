...
# def ltalicdfonts(size):
#     global c
#     c.setFont("MyOwnArialItalic", size)
...
def textsize(c,result):
    ...
    if len(invoiceno) == 1:
        ...
        if challanno[-1] != challanno[-2]:
            ...
            # c.drawAlignedString(425, t, str("%.3f" % float(GTotalweight)))
            # t=0
            # GTotalweight=0
            if printcounter<=6:
                # printparticular(result,"")
                ...
                printcounter=0
                c.drawString(520, 700, "Page no : " + str(pageno))
                p = page()
                c.drawString(467, 290, "Continued  on Page No. " + str(p))
                c.showPage()
                # p = page()
                fonts(9)
        ...
        # c.drawAlignedString(425, t, str(t)+'-'+str("%.3f" % float(GTotalweight)))
        # t=0
        GTotalweight=0
        pageno=0
        printtotalmain()
        getandprinttaxdeatils(invoiceno[-2],roundoff[-2])
        # printfooter(result)
...
def header(result):
    ...
    # print("getting the code for the logo printing : "+result['INVOICENO']+": -- "+str(result['DIVISIONCODE']))
    ...
    c.drawCentredString(305, 800, result['COMPANYNAME']+" "+result['DIVISIONCODE'])
    fonts(8)
    ...
    # c.drawString(25, ca-35, "\u0332".join("CONSIGNEE : (SHIP TO - Address of Delivery)"))
    c.drawString(25, ca-35, "\u0332".join("CONSIGNEE : (SHIP TO - Address of Delivery)"))
    # c.line(35, 820, ca-35, 820)
    c.drawString(25, ca-45, str(result['CONSIGNEENAME']))
    # ca = 575
    ca = ca-55
...
def border(c, result):
    ...
    # c.line(675, 675, 260, 6
    # 75)  # center vertical line  short line for address and header deatils  |

    c.line(20, 545, 580,545)  # line after Ref  horizontal line
    # c.drawString(30, 536, "CHALLAN No.")
    # c.drawString(100, 536, "Item Description")
    ...
    # c.drawString(480, 526, "RATE")
    # c.drawString(85, 516, "EWAY BILL NUMBER / DATE")
    c.line(20, 511, 580, 511)  # fourth horizontal line
    ...
    printmessage()
    c.line(20, 145, 580, 145)  # fifth horizontal line before message
...
def printparticular(result,MSG):
    ...
    # c.drawAlignedString(555, d, str(format_currency("%.2f" % float(result['ITEMAMOUNT']), '', locale='en_IN')))
...
def printInvoiceBasicValue(BASICVALUE,p):
    c.drawString(350, p, "Total Basic Value : ")
...
def printbankandtaxdetails(result):
    ...
    c.drawString(380, 180, " : "+str(result['BANKNAME']))
    c.drawString(310, 170, "BANK BRANCH")
    c.drawString(380, 170, " : "+str(result['BRANCHNAME']))
    c.drawString(310, 160, "ACCOUNT No.")
    c.drawString(380, 160, " : "+str(result['ACCOUNTNUMBER']))
    c.drawString(310, 150, "IFC Code")
    c.drawString(380, 150, " : "+str(result['BANKIFSCCODE']))
...
def total(resultitemamount):
    global Amount
    global totaltaxablevalue
    print(resultitemamount['CALCULATEDVALUE'])
    Amount = Amount + float("%.2f" % float(resultitemamount['CALCULATEDVALUE']))
    totaltaxablevalue=totaltaxablevalue+float("%.2f" % float(resultitemamount['CALCULATEDVALUE']))
...
def getandprinttaxdeatils(invoicenotax,roundoffvalue):
    ...
    c.drawString(350, p, "Total Taxable Value : ")

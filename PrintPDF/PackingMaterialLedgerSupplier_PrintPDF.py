...
def header(stdt,etdt,d,result,plantcode):
    ...
    c.drawString(470, 750, str("ISS. QTY"))
    c.drawString(520, 750, str("BALANCE"))
    c.drawString(10, 730, str("SUPPLIER:"))
    c.drawString(70, 730, str(result['SUPPLIER']))
    c.drawString(300, 730, str("PALLETE TYPE:"))
    f = 730
    if len(str(result['PALLETENAME'])) > 45:

        lines = textwrap.wrap(str(result['PALLETENAME']), 45, break_long_words=False)
        for i in lines:
            c.drawString(370, f, str(i))
            f = f - 10
    else:
        c.drawString(370, f, str(result['PALLETENAME']))
        f = f - 10
...
def data(stdt,etdt,result,d):
    ...
    no = no + 1
    fonts(7)
    c.drawString(10, d, str(result['GATEPASSDATE'].strftime('%d-%m-%Y')))
    c.drawString(50, d, str(result['TYPEOF']))
    c.drawString(100, d, str(result['GATEPASSNO']))
    c.drawString(180, d, str(""))
    c.drawString(280, d, str(""))
    c.drawAlignedString(430, d, str(("%.0f" % float(result['RECDQTY']))))
    c.drawAlignedString(490, d, str(("%.0f" % float(result['ISSQTY']))))
...
def total(result):
    ...
    GrandReceivedTotal=GrandReceivedTotal+float("%.0f" % float(result['RECDQTY']))
    GrandIssuedTotal=GrandIssuedTotal+float("%.0f" % float(result['ISSQTY']))
    ReceivedQuantityTotal=ReceivedQuantityTotal+float("%.0f" % float(result['RECDQTY']))
    IssuedQuantityTotal = IssuedQuantityTotal + float("%.0f" % float(result['ISSQTY']))
...
def companyclean():
    global GrandReceivedTotal, ReceivedQuantityTotal
    global GrandIssuedTotal, IssuedQuantityTotal
    GrandReceivedTotal=0
    GrandIssuedTotal=0
    ReceivedQuantityTotal = 0
    IssuedQuantityTotal = 0
...
def textsize(c, result, d, stdt, etdt):
    ...
    if len(plantcode)==1:
        ...
        if itemtype[-1] == itemtype[-2]:
            ...
            fonts(9)
            d = dvalue(stdt, etdt, result, plantcode)
            c.drawString(10, d, str("SUPPLIER:"))

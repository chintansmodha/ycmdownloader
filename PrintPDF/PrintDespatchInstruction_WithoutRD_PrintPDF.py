...
def header(stdt,etdt,result,divisioncode):
    ...
    c.drawString(430, 570, "Lot No.")
    c.drawString(480, 570, "Qnty Kgs.")
    c.drawString(550, 570, "Rate")
...
def data(stdt,etdt,result,d):
    ...
    c.drawString(450, d, str(result['LOTNO']))
    c.drawRightString(515, d, str(result['QUANTITY']))
    c.drawString(160, d, str(result['SHADECODE']))
    if result['RATEWITHOUTDHARA'] != None:
        c.drawRightString(570, d, str(("%.2f" % float(result['RATEWITHOUTDHARA']))))

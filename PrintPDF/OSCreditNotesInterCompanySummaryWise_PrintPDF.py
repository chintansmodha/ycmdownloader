...
def dvalue(asondt, divisioncode):
    global d
    if d > 40:
        d = d - 10
        return d
        ...
        c.setPageSize(landscape(A4))
        c.showPage()
        header(asondt, divisioncode,agentgroup)
        return d
...
def partytotal(result):
    ...
    TotalPartyAmt=TotalPartyAmt+(float("%.2f"%float(result['INVAMT'])))
...
def brokertotal(result):
    ...
    if result['INVAMT']!=None:
        TotalBrokerAmt=TotalBrokerAmt+(float("%.2f"%float(result['INVAMT'])))

def partytotalprint(d,asondt):
    global TotalPartyQty
    global TotalPartyAmt
...
def logic(result):
    ...
    agentgroup.append(result['BROKER'])
...
def header(asondt,divisioncode,agentgroup):
    fonts(9)
    c.setFillColorRGB(0, 0, 0)
    c.drawCentredString(450, 580, divisioncode[-1]) 
    fonts(8)
    c.drawCentredString(450, 560, "PartyWiseAgentLiftingMiniatureCopyNo Till" + asondt.strftime('%d-%m-%Y'))
...
def data(result,d):
    ...
    if result['INVOICEDATE']!=None:
        c.drawString(80, d, str(result['INVOICEDATE']))
    if result['PARTY']!=None:
        c.drawString(160, d, str(result['PARTY']))
    # if result['ITEM']!=None:
    ...
    if result['OSAMT']!=None:
        c.drawAlignedString(710, d, str(result['OSAMT']))
    if result['INVAMT']!=None:
        c.drawAlignedString(805, d, str(result['INVAMT']))
    partytotal(result)
    brokertotal(result)



def textsize(c,result, d, asondt):
    d=dvalue(asondt, divisioncode)
    logic(result)

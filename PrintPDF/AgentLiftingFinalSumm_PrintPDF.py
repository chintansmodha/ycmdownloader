...
def header(stdt,etdt, divisioncode):
    ...
    c.drawCentredString(300, 780, "Agent Wise Lifting Final Summary From  " + str(stdt.strftime(' %d  %b  %Y')) +
...
def CompTotal(result):
    global compAmt, compQnt
    compAmt = compAmt + float(result['AMOUNT'])
    compQnt = compQnt + float(result['QUANTITY'])
...
def CompTotalPrint():
    global compAmt, compQnt
    c.drawAlignedString(565, d, str('{0:1.3f}'.format(compAmt)))
    c.drawAlignedString(485, d, str('{0:1.3f}'.format(compQnt)))
    compAmt = 0
    compQnt = 0
        ...
        if BrokerGroup[-1] == BrokerGroup[-2]:
            if Broker[-1] == Broker[-2]:
                if Party[-1] == Party[-2]:
                    ...
                    boldfonts(7)
                    c.drawString(300, d , "Party Total: ")
                    PartyTotalPrint()
                    d = dvalues(c, stdt, etdt, divisioncode)
                    d = dvalues(c, stdt, etdt, divisioncode)
                    d = dvalues(c, stdt, etdt, divisioncode)

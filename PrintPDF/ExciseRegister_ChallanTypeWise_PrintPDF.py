...
def header(stdt,etdt,divisioncode):
    ...
    c.drawString(110, 765, "ChallanNo.")
    c.drawString(210, 765, "Inv.Date")
    c.drawString(310, 765, "QTY")
...
def data(result,d):
    ...
    if result['INVNO']!=None:
        c.drawString(20, d, str(result['INVNO']))
    if result['CHALLANNO']!=None:
        c.drawString(110, d, str(result['CHALLANNO']))
    if result['INVDATE']!=None:
        c.drawString(210, d, str(result['INVDATE'].strftime('%d-%m-%Y')))
    if result['QTY']!=None:
        c.drawAlignedString(310, d, str(result['QTY']))
    ...
    if result['ASSAMT']!=None:
        c.drawAlignedString(580, d, str(result['ASSAMT']))
    if result['FRT']!=None:
        c.drawAlignedString(670, d, str(result['FRT']))
    if result['INS']!=None:
        c.drawAlignedString(740, d, str(result['INS']))
    if result['OTHCH']!=None:
        c.drawAlignedString(810, d, str(result['OTHCH']))
    if result['IGST']!=None:
        c.drawAlignedString(880, d, str(result['IGST']))
    if result['CGST']!=None:
        c.drawAlignedString(920, d, str(result['CGST']))
    if result['UTGST']!=None:
        c.drawAlignedString(970, d, str(result['UTGST']))
    if result['GST']!=None:
        c.drawAlignedString(1020, d, str(result['GST']))
    if result['INVAMT']!=None:
        c.drawAlignedString(1080, d, str(result['INVAMT']))
    
    # c.drawAlignedString(1080, d, " ")
    # if result['TCS']!=None:

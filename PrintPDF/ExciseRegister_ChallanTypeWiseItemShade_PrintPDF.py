...
def header(stdt,etdt,divisioncode):
    ...
    c.drawString(220, 765, "Inv.Date")
    c.drawString(300, 765, "QTY")
...
def data(result,d):
    ...
    if result['INVDATE']!=None:
        c.drawString(200, d, str(result['INVDATE'].strftime('%d-%m-%Y')))
    if result['QTY']!=None:
    ...
    if result['FRT']!=None:
        c.drawAlignedString(660, d, str(result['FRT']))
    if result['INS']!=None:
        c.drawAlignedString(730, d, str(result['INS']))
    if result['OTHCH']!=None:
        c.drawAlignedString(800, d, str(result['OTHCH']))
    if result['IGST']!=None:
        c.drawAlignedString(870, d, str(result['IGST']))
    if result['CGST']!=None:
        c.drawAlignedString(910, d, str(result['CGST']))
    if result['UTGST']!=None:
        c.drawAlignedString(960, d, str(result['UTGST']))
    if result['GST']!=None:

...
from PrintPDF import BrokerWiseRD_WithoutRDInv_PrintPDF as pdfBWRDWRI
...
def BrokerWiseRD_PrintPDF(LSCompanyCode, LCCompanyCode, LMCompanyCode, LSBranch, LCBranch, LSBrokerGroup, LCBrokerGroup,
    ...
    sql = "Select          Company.LONGDESCRIPTION As Company " \
          ...
          "And     DharaRate.value <= 0 Or DharaRate.Value is null " \
    ...
    while result != False:
        ...
        pdfBWRDWRI.textsize(pdfBWRDWRI.c, result, pdfBWRDWRI.d,stdt,etdt)
        pdfBWRDWRI.d=pdfBWRDWRI.dvalue()
        result = con.db.fetch_both(stmt)

        if pdfBWRDWRI.d<65 :
            # pdfBWRDWRI.d=735
            pdfBWRDWRI.c.setPageSize(pdfBWRDWRI.landscape(pdfBWRDWRI.A4))
            pdfBWRDWRI.c.showPage()
            pdfBWRDWRI.header(pdfBWRDWRI.divisioncode,stdt,etdt)
            pdfBWRDWRI.fonts(7)
    ...
    pdfBWRDWRI.c.showPage()
    pdfBWRDWRI.c.save()
    pdfBWRDWRI.newrequest()
    pdfBWRDWRI.d = pdfBWRDWRI.newpage()
...
from PrintPDF import ItemWise_Summary_PrintPDF as pdfrpt
from Global_Files import Connection_String as con
from datetime import datetime
counter=0
def ItemWise_Summary_PrintPDF(LSDepartment, LSSupplier, LSItemType, LSItems, LDStartDate, LDEndDate, LCDepartment,LCSupplier, LCItemType, LCItem, LSFileName, LSReportType):
    ...
    sql = "select DIVISION.LONGDESCRIPTION AS DIVCODE," \
          "PRODUCT.LONGDESCRIPTION AS ITEM,MRNDETAIL.PRIMARYQTY AS QUANTITY,UNITOFMEASURE.LONGDESCRIPTION AS UNIT," \
          "MRNDETAIL.BASICVALUE AS AMOUNT,MRNDETAIL.UNITPRICE AS MRNRATE,COSTCENTER.LONGDESCRIPTION AS STORE," \
          "ITEMTYPE.LONGDESCRIPTION AS ITEMTYPE,PURCHASEORDERLINE.PRICE AS PORATE " \
          "FROM MRNHEADER " \
         ...
         "JOIN    FullItemKeyDecoder FIKD         ON      MRNDETAIL.ITEMTYPEAFICODE  = FIKD.ITEMTYPECODE " \
          ...
          "JOIN ITEMTYPE ON MRNDETAIL.ITEMTYPEAFICODE=ITEMTYPE.CODE " \
          ...
          "where MRNHEADER.MRNDATE between ? and ? "+Department+Supplier+ItemType+Item+" And MRNHEADER.INVOICENO is not null " \
          "order by DIVISION.LONGDESCRIPTION,COSTCENTER.LONGDESCRIPTION,ITEMTYPE.LONGDESCRIPTION,PRODUCT.LONGDESCRIPTION"
    ...
    if result == False:
        if counter > 0:
            ...
            pdfrpt.fonts(7)
            pdfrpt.printitemtotallast()
            pdfrpt.d = pdfrpt.dvalue(stdt, etdt, pdfrpt.divisioncode)
            pdfrpt.printstoretotallast()
            pdfrpt.d = pdfrpt.dvalue(stdt, etdt, pdfrpt.divisioncode)

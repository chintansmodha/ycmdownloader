...
def PrintChallanPDF(request):
    ...
    # save_name = os.path.join(os.path.expanduser("~"),"D:/Report Development/Generated Reports/Print Yarn Challan/", LSFileName)
...
def PrintPDF(LSChallanNo,LSChallanDate,LSLotNo,LSLrNo,LSLrDt,LSQuantity,LSBoxes,Company,Party):
    sql = " select BUnit.LONGDESCRIPTION AS Unit, Plant.ADDRESSLINE1 As Address, SD.PROVISIONALCODE As ChallanNo," \
          " PRODUCTIE.TARIFFCODE As HSNCD,  ADG.GSTINNUMBER As GSTIN," \
          ...
          " ADDRESSGST.PROVISIONALGSTINNUMBER," \
          " ADDRESS.ADDRESSEE As PARTYNAME," \
          " NOTE.NOTE As REMARKS," \
          " VARCHAR_FORMAT(SD.PROVISIONALDOCUMENTDATE, 'DD-MM-YYYY') As ChallanDate," \
          " 0 As LotNO, SD.EXTERNALREFERENCE As LRNO," \

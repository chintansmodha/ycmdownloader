...
from PrintPDF import PrintProformaBL_PrintPDF as pdfrpt
save_name=""
...
def PrintPDF(InvoiceNos, InvoiceDts, startdate, enddate):
    ...
    sql = "SELECT Comp.LONGDESCRIPTION As Company " \
          ...
          ", 'TEL NO. : '||Coalesce(''||VARCHAR(CORP_ADDRESS.ADDRESSPHONENUMBER),'') As PhoneNO  " \
          ", 'TAX ID NO.: '||''||FIRM.PANNO ||'                '|| 'GSTIN  : 27AAACB2976M1ZB ' As CROP_GSTIN_TAXID  " \
          ", 'CIN: '||COALESCE(''||VARCHAR(Firm.TINNo),'') ||'        '|| 'IEC.NO.: ' ||COALESCE(''||VARCHAR(Firm.IECODE),'') As CINIecNo " \
          ", PLANTINVOICE.CODE ||'             DT :   '|| VARCHAR_FORMAT(PLANTINVOICE.INVOICEDATE,'DD-MM-YYYY') AS INVOICENOandDt " \
          ", BILLOFLADING.CONTAINERNO ||'             DT :   '|| VARCHAR_FORMAT(BILLOFLADING.BILLOFLADINGDATE,'DD-MM-YYYY') AS BookingNo " \
          ...
          ", COALESCE(PLANTINVOICE.PLACEOFRECEIPTBYPRECARRIAGE,'') As PlaceOfReceipt " \
          ", COALESCE(PlaceDelv.LONGDESCRIPTION,'') PlaceOfDelv " \
          ...
          ", (SELECT COALESCE(CAST(Sum(IndTaxDetail.VALUE) AS DECIMAL(20,4)),0.0000) FROM IndTaxDetail " \
          ...
          "AND INDTAXDETAIL.AbsUniqueID = PLANTINVOICE.AbsUniqueID) As FreighttValue " \
          ", Cast(PlantInvoice.BASICVALUE As Decimal(20,4)) As ORDERTOTAL " \
          ", PLANTINVOICE.ORDERCURRENCYCODE AS CURRENCY" \
          ", PLANTINVOICE.EXCHANGERATEOFCONTRACT As EXRATE " \
          ", PlaceIssueBl.LONGDESCRIPTION As PlaceOfIssueBl " \
          ", BILLOFLADING.BILLOFLADINGDATE As DateOfIssueBL " \
          ", Coalesce(BILLOFLADING.TYPEOFSERVICE,'') As NoOfSeq " \
          ...
          "Left Join BILLOFLADING                        On      CUSTOMINVOICE.AWBNOCODE = BILLOFLADING.CODE " \
          "Left Join DESTINATION PlaceIssueBl            On      BILLOFLADING.PLACECODE = PlaceIssueBl.CODE   " \
          ...
          ", BILLOFLADING.CONTAINERNO ||'             DT :   '|| VARCHAR_FORMAT(BILLOFLADING.BILLOFLADINGDATE,'DD-MM-YYYY') " \
          ...
          ", PIL.NUMBEROFBALES " \
          ", '' " \
          ", CORP_ADDRESS.ADDRESSPHONENUMBER " \
          ", 'CIN: '||COALESCE(''||VARCHAR(Firm.TINNo),'') ||'        '|| 'IEC.NO.: ' ||COALESCE(''||VARCHAR(Firm.IECODE),'') " \
          ", 'TAX ID NO.: '||''||FIRM.PANNO ||'                '|| 'GSTIN  : 27AAACB2976M1ZB ' " \
          ", PLANTINVOICE.PLACEOFRECEIPTBYPRECARRIAGE , PlaceDelv.LONGDESCRIPTION, PLANTINVOICE.AbsUniqueID " \
          ", PlantInvoice.BASICVALUE, PLANTINVOICE.ORDERCURRENCYCODE, PLANTINVOICE.EXCHANGERATEOFCONTRACT " \
          ", PlaceIssueBl.LONGDESCRIPTION " \
          ", BILLOFLADING.BILLOFLADINGDATE " \
          ", BILLOFLADING.TYPEOFSERVICE " \
          ", PlantInvoice.CATEGORY " \
          ", PlantInvoice.INVOICEDATE " \
          "Order By Company, INVOICENOandDt, itmTyp, lotNo, Item "
    # try:
    ...
    if result == False:
        if counter > 0:
              ...
              pdfrpt.d = pdfrpt.dvalue()
              pdfrpt.c.drawString(175, pdfrpt.d, "LOT NO.: " + str(pdfrpt.lotno[-1]))
              pdfrpt.d = pdfrpt.dvalue()
              pdfrpt.d = pdfrpt.dvalue()
              pdfrpt.c.drawString(175, pdfrpt.d, "HSNCODE : " + pdfrpt.hsncode[-1])
              pdfrpt.d = pdfrpt.dvalue()
              pdfrpt.d = pdfrpt.dvalue()
              pdfrpt.c.drawString(175, pdfrpt.d, "COUNTRY OF ORIGIN:  " + str(pdfrpt.countryofOrigin[-1]).upper())
              ...
              pdfrpt.c.drawString(94, 442, str('{0:1.3f}'.format(pdfrpt.grosswt)))
              pdfrpt.c.drawString(94, 432, str('{0:1.3f}'.format(pdfrpt.netwt)))
              pdfrpt.c.drawString(94, 412, str(pdfrpt.packages))
              pdfrpt.boldfonts(6)
              pdfrpt.c.drawCentredString(185, 482, str(pdfrpt.packages))  # No Of Cartons
              pdfrpt.fonts(7)
              pdfrpt.TotalClean()

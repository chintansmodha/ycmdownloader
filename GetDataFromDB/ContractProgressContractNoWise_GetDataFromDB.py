...
def ContractProgress_PrintPDF(LSBroker, LCBroker, LSYarn, LCYarn, LSItem, LCItem, LSShade, LCShade, LSParty, LCParty, LDStartDate,LDEndDate):
    ...
    sql = "Select   PLANT.LONGDESCRIPTION As Company " \
          ...
          ", ContSol.ITEMTYPEAFICODE As ItemTyp " \
          ...
          "Order BY Company,ContNo, Broker, ItemTyp, Item, Shade, OrdNo, OrdQty"
    ...
    while result != False:
        ...
        pdfCPCNW.textsize(pdfCPCNW.c, result, pdfCPCNW.d,stdt,etdt)
        pdfCPCNW.d=pdfCPCNW.dvalue()
        result = con.db.fetch_both(stmt)

        if pdfCPCNW.d<65 :
            pdfCPCNW.k = pdfCPCNW.k + 1
            pdfCPCNW.c.setPageSize(pdfCPCNW.landscape(pdfCPCNW.A4))
            pdfCPCNW.c.showPage()
            pdfCPCNW.d = pdfCPCNW.dvalueset()
            pdfCPCNW.header(stdt,etdt,pdfCPCNW.divisioncode)
            pdfCPCNW.fonts(7)
    ...
    if result == False:
        if counter > 0:
            ...
            pdfCPCNW.d = pdfCPCNW.dvalue()
            if int(pdfCPCNW.ConOrdQty) != 0:
                pdfCPCNW.ContractTotalPrint(pdfCPCNW.d)
                pdfCPCNW.d = pdfCPCNW.dvalue()
                pdfCPCNW.d = pdfCPCNW.dvalue()
                pdfCPCNW.ContractClean()
            # pdfCPCNW.BrokerGroupTotalPrint(pdfCPCNW.d)
            # pdfCPCNW.d = pdfCPCNW.dvalue()
            # pdfCPCNW.d = pdfCPCNW.dvalue()
            pdfCPCNW.BrokerGroupClean()

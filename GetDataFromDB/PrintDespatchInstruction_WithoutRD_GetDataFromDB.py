...
from PrintPDF import PrintDespatchInstruction_WithoutRD_PrintPDF as pdfrpt
save_name=""
counter=0

def PrintDespatch_WithoutRD_PDF(request):
    global save_name
    LSName = datetime.now()
    LSstring = str(LSName)
    LSFileName = LSstring[0:4] + LSstring[5:7] + LSstring[8:10] + LSstring[11:13] + LSstring[14:16] + LSstring[17:19] + LSstring[20:]
    LSFileName = "PrintDespatchInstructionWithoutRD" + LSFileName
    save_name = os.path.join(os.path.expanduser("~"),
                             "D:/Report Development/Generated Reports/Print Despatch Instruction Without RD/",
                             LSFileName)
...
def PrintPDF(LSDespatchNo,LSDespatchDate,LSDocumentType):
    sql = " SELECT   Company.LONGDESCRIPTION AS Company" \
          " , ITD.RateWithoutDhara As RATEWITHOUTDHARA" \
          ...
          " WHERE SOL.DOCUMENTTYPETYPE IN ('"+LSDocumentType[0]+"')" +LSDespatchNo+LSDespatchDate+"" \

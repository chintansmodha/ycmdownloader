...
def PendingDespatchInstruction_PrintPDF(LSCompanyUnitCode, LSParty, LDstdt, LDEndDate, LCParty, LCCompanyCode, LSDocumentType,
    ...
    sql ="Select  Division.LongDescription As BUSINESSUNITName" \
         ...
         "order by Division.LongDescription,Broker, ordDate, ordNo"
    ...
    while result != False:
        ...
        pdfPDI.textsize(pdfPDI.c, result, pdfPDI.d,stdt,etdt,LCRemarks)
        pdfPDI.d=pdfPDI.dvalue()
        result = con.db.fetch_both(stmt)

        if LCRemarks == ['1']:
            if pdfPDI.d<50 :
                pdfPDI.c.showPage()
                pdfPDI.header(stdt,etdt,pdfPDI.divisioncode)
                pdfPDI.d = pdfPDI.newpage()
                pdfPDI.fonts(8)
        else:
            if pdfPDI.d<40 :
                # pdfPDI.d=735
                #pdfPDI.c.setPageSize(pdfPDI.landscape(pdfPDI.A3))
                pdfPDI.c.showPage()
                pdfPDI.header(stdt,etdt,pdfPDI.divisioncode)
                pdfPDI.d = pdfPDI.newpage()
                pdfPDI.fonts(8)
    ...
    if result == False:
        if counter > 0:
            ...
            if LCRemarks == ['1']:
                if pdfPDI.Remark == '':
                    pdfPDI.c.drawString(19, pdfPDI.d, "Remarks :")
                else:
                    pdfPDI.c.drawString(19, pdfPDI.d, pdfPDI.Remark)
                pdfPDI.dvalue()
            pdfPDI.c.drawAlignedString(400, pdfPDI.d, "Broker Total: ")
            pdfPDI.c.drawAlignedString(465, pdfPDI.d, str(pdfPDI.format_number(float(pdfPDI.Broker_Total), locale='en_IN')))
            pdfPDI.c.drawAlignedString(580, pdfPDI.d, str(pdfPDI.format_number(float(pdfPDI.BrokerBAL_Total), locale='en_IN')))
            pdfPDI.dvalue()
            pdfPDI.c.drawAlignedString(400, pdfPDI.d, "Grand Total: ")
            pdfPDI.c.drawAlignedString(465, pdfPDI.d,str(pdfPDI.format_number(float(pdfPDI.Grand_Total), locale='en_IN')))
            pdfPDI.c.drawAlignedString(580, pdfPDI.d,str(pdfPDI.format_number(float(pdfPDI.GrandBal_Total), locale='en_IN')))

...
def FinishedStockInHand_PrintPDF(LSDepartmentCode, LCDepartmentCode, LSQuality, LCQuality, LSWinding,
                                 LCWinding, LSLotNo, LCLotNo, LSItem, LCItem, LSItmtype, LCItmtype, LDStartDate,
                                 LDEndDate):
    ...
    sql = "Select          FINBUSINESSUNIT.LONGDESCRIPTION As CompName " \
          ...
          "And     ELEMENTS.ENTRYDATE       <=     " + enddate + " " + Qualitys + " " + Windings + " " + LotNos + " " + LSItems + " " + LSItmtypes + " " + Departmentcodes + " " \
          ...
          "Order By CompName, Department, Product, ShadeName "
    ...
    while result != False:
        ...
        pdfFSIHSIS.textsize(pdfFSIHSIS.c, result, pdfFSIHSIS.d, stdt, etdt)
        pdfFSIHSIS.d = pdfFSIHSIS.dvalue()
        result = con.db.fetch_both(stmt)

        if pdfFSIHSIS.d < 30:
            pdfFSIHSIS.d = 745
            pdfFSIHSIS.c.showPage()
            pdfFSIHSIS.header(stdt, etdt, pdfFSIHSIS.divisioncode)
            pdfFSIHSIS.fonts(7)
    ...
    if result == False:
        if counter > 0:
            ...
            pdfFSIHSIS.d = pdfFSIHSIS.dvalue()
            pdfFSIHSIS.ItewiseTotalPrint(pdfFSIHSIS.d)
            pdfFSIHSIS.d = pdfFSIHSIS.dvalue()
            pdfFSIHSIS.d = pdfFSIHSIS.dvalue()
            pdfFSIHSIS.DepartmentTotalPrint(pdfFSIHSIS.d)
            pdfFSIHSIS.d = pdfFSIHSIS.dvalue()
            pdfFSIHSIS.d = pdfFSIHSIS.dvalue()
            pdfFSIHSIS.CompanyTotalPrint(pdfFSIHSIS.d)
            pdfFSIHSIS.ItemWiseClean()
            pdfFSIHSIS.DepartmentClean()
            pdfFSIHSIS.CompanyClean()
    ...
    pdfFSIHSIS.newrequest()
    pdfFSIHSIS.d = pdfFSIHSIS.newpage()
    pdfFSIHSIS.ItemWiseClean()
    pdfFSIHSIS.DepartmentClean()
    pdfFSIHSIS.CompanyClean()
...
def PendingDespatchInstructionItemwise_PrintPDF(LSCompanyUnitCode, LSParty, LDstdt, LDEndDate, LCParty, LCCompanyCode, LSDocumentType,
    ...
    while result != False:
        ...
        pdfPDII.textsize(pdfPDII.c, result, pdfPDII.d,stdt,etdt,LCRemarks)
        pdfPDII.d=pdfPDII.dvalue()
        result = con.db.fetch_both(stmt)
        ...
        if LCRemarks == ['1']:
            if pdfPDII.d<50 :
                pdfPDII.d=745
                pdfPDII.c.showPage()
                pdfPDII.header(stdt,etdt,pdfPDII.divisioncode)
                pdfPDII.fonts(7)
        else:
            if pdfPDII.d<40 :
                pdfPDII.d=745
    ...
    if result == False:
        if counter > 0:
            ...
            if LCRemarks == ['1']:
                if pdfPDII.Remark == '':
                    pdfPDII.c.drawString(19, pdfPDII.d, "Remarks :")
                else:
                    pdfPDII.c.drawString(19, pdfPDII.d, pdfPDII.Remark)
                pdfPDII.dvalue()
            pdfPDII.c.drawAlignedString(400, pdfPDII.d, "Item Total: ")
            pdfPDII.c.drawAlignedString(465, pdfPDII.d, str(pdfPDII.format_number(float(pdfPDII.Item_Total), locale='en_IN')))
            pdfPDII.c.drawAlignedString(580, pdfPDII.d, str(pdfPDII.format_number(float(pdfPDII.ItemBAL_Total), locale='en_IN')))
            pdfPDII.dvalue()
            pdfPDII.c.drawAlignedString(400, pdfPDII.d, "Grand Total: ")
            pdfPDII.c.drawAlignedString(465, pdfPDII.d,str(pdfPDII.format_number(float(pdfPDII.Grand_Total), locale='en_IN')))
            pdfPDII.c.drawAlignedString(580, pdfPDII.d,str(pdfPDII.format_number(float(pdfPDII.GrandBal_Total), locale='en_IN')))
            ...
            return
    #pdfPDII.workbook.close()
    # pdfPDII.c.setPageSize(pdfPDII.landscape(pdfPDII.A3))

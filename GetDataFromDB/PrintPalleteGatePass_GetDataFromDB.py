...
from PrintPDF import PrintPalleteGatePass_PrintPDF as pdfrpt
from ProcessSelection import PrintPalleteGatePass_ProcessSelection as PPGPPS
save_name=""
counter=0
def PrintPalleteGatePassPDF(request):
    global save_name
    LSName = datetime.now()
    LSstring = str(LSName)
    LSFileName = LSstring[0:4] + LSstring[5:7] + LSstring[8:10] + LSstring[11:13] + LSstring[14:16] + LSstring[17:19] + LSstring[20:]
    LSFileName = "PrintPalleteGatePass" + LSFileName
    save_name = os.path.join(os.path.expanduser("~"),"D:/Report Development/Generated Reports/Print Pallete Gate Pass/",LSFileName)
    ...
    if not os.path.isfile(filepath):
        return render(request, 'PrintPalleteGatePassTable.html', {'GDataPrintPalleteGatePass': PPGPPS.GDataPrintPalleteGatePass,
                                                                      'Exception': PPGPPS.Exceptions})
...
def  PrintPDF(LSGatePassNo, LSGatePassDate):
    sql="SELECT  BUNIT.LongDescription AS COMPANYNAME" \
        ...
        " WHERE ID.DocumentTypeType = '05'  AND ID.TEMPLATECODE='PMS'" +LSGatePassNo+"" \

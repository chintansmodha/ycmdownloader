...
def ContractProgress_PrintPDF(LSBroker, LCBroker, LSYarn, LCYarn, LSItem, LCItem, LSShade, LCShade, LSParty, LCParty, LDStartDate,LDEndDate):
    ...
    sql = "Select   PLANT.LONGDESCRIPTION As Company " \
          ...
          "Order BY Company, Broker, Party, ItemTyp, Item, ContNo, Shade, OrdNo, OrdQty"
    ...
    while result != False:
        ...
        if pdfCP.d<65 :
            # pdfCP.d=735
            pdfCP.k = pdfCP.k + 1
            pdfCP.c.setPageSize(pdfCP.landscape(pdfCP.A4))
            pdfCP.c.showPage()
            pdfCP.dvalueset()
    ...
    if result == False:
        if counter > 0:
            ...
            pdfCP.d = pdfCP.dvalue()
            if int(pdfCP.ConOrdQty) != 0:
                pdfCP.ContractTotalPrint(pdfCP.d)
                pdfCP.d = pdfCP.dvalue()
                pdfCP.d = pdfCP.dvalue()
                pdfCP.ContractClean()
            pdfCP.BrokerGroupTotalPrint(pdfCP.d)
            pdfCP.d = pdfCP.dvalue()
            pdfCP.d = pdfCP.dvalue()
            pdfCP.BrokerGroupClean()

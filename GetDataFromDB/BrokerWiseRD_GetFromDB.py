...
def BrokerWiseRD_PrintPDF(LSCompanyCode, LCCompanyCode, LMCompanyCode, LSBranch, LCBranch, LSBrokerGroup, LCBrokerGroup,
    ...
    while result != False:
        ...
        pdfBWRD.textsize(pdfBWRD.c, result, pdfBWRD.d,stdt,etdt)
        pdfBWRD.d=pdfBWRD.dvalue()
        result = con.db.fetch_both(stmt)

        if pdfBWRD.d<65 :
            # pdfBWRD.d=735
            pdfBWRD.c.setPageSize(pdfBWRD.landscape(pdfBWRD.A4))
            pdfBWRD.c.showPage()
            pdfBWRD.header(pdfBWRD.divisioncode,stdt,etdt)
            pdfBWRD.fonts(7)

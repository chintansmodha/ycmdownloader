...
def ProductionSummary_PrintPDF(LSDepartmentCode, LCDepartmentCode, LSProduction, LCProduction, LSLotNo, LCLotNo,LSItm, LCItm,
    ...
    sql = "Select          FINBUSINESSUNIT.LONGDESCRIPTION As CompName " \
          ...
          ", CAST(SUM(Case When ELEMENTS.ENTRYDATE  >= "+startdate+" Then BKLELEMENTS.ACTUALNETWT Else 0 End) As DECIMAL(10,3)) As NetWt " \
          "From BKLELEMENTS " \
          ...
          "Order By CompName,PRODUCT, Dates asc "


    sql2 ="Select          FINBUSINESSUNIT.LONGDESCRIPTION As CompName " \
          ", Varchar_FORMAT(ELEMENTS.ENTRYDATE, 'YYYY-MM ') As Dates " \
          ", CAST(SUM(Case When ELEMENTS.ENTRYDATE  >= "+startdate+" Then BKLELEMENTS.ACTUALNETWT Else 0 End) As DECIMAL(10,3)) As NetWts " \
          "From BKLELEMENTS " \
          "Join    PLANT                           ON      BKLELEMENTS.PLANTCODE = PLANT.CODE " \
          "Join    BUSINESSUNITVSCOMPANY           On      PLANT.CODE = BUSINESSUNITVSCOMPANY.FACTORYCODE " \
          "Join    FINBUSINESSUNIT                 ON      BUSINESSUNITVSCOMPANY.BUSINESSUNITCODE = FINBUSINESSUNIT.CODE " \
    ...
    while result != False:
        ...
        pdfPSD.textsize_Nodept(pdfPSD.c, result, pdfPSD.d,stdt,etdt,monthwisetotal,Date)
        ...
        if pdfPSD.d<30 :
            ...
            pdfPSD.c.showPage()
            pdfPSD.header_Nodept(stdt,etdt,pdfPSD.divisioncode)
            pdfPSD.fonts(7)
    ...
    if result == False:
        if counter > 0:
            ...
            pdfPSD.d = pdfPSD.dvalue()
            pdfPSD.boldfonts(7)
            pdfPSD.c.drawString(10, pdfPSD.d, "Total :")
            h = 290
            startdate = str(stdt)
            enddate = str(etdt)

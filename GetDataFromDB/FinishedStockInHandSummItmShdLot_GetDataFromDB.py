...
def FinishedStockInHand_PrintPDF(LSDepartmentCode, LCDepartmentCode, LSQuality, LCQuality, LSWinding,
    ...
    sql = "Select          COALESCE(COSTCENTER.LONGDESCRIPTION,'') As Department " \
          ...
          "Order By Department, ItmType, Product, ShadeName, LotNo "
    ...
    while result != False:
        ...
        pdfFSIHSISL.textsize(pdfFSIHSISL.c, result, pdfFSIHSISL.d,stdt)
        pdfFSIHSISL.d=pdfFSIHSISL.dvalue()
        result = con.db.fetch_both(stmt)

        if pdfFSIHSISL.d<30 :
            pdfFSIHSISL.d=745
            pdfFSIHSISL.c.showPage()
            pdfFSIHSISL.header(stdt,pdfFSIHSISL.divisioncode)
            pdfFSIHSISL.fonts(7)
    ...
    if result == False:
        if counter > 0:
            ...
            pdfFSIHSISL.d = pdfFSIHSISL.dvalue()
            pdfFSIHSISL.ItemwiseTotalPrint(pdfFSIHSISL.d)
            pdfFSIHSISL.d = pdfFSIHSISL.dvalue()
            pdfFSIHSISL.d = pdfFSIHSISL.dvalue()
            pdfFSIHSISL.ItemTypeTotalPrint(pdfFSIHSISL.d)
            pdfFSIHSISL.d = pdfFSIHSISL.dvalue()
            pdfFSIHSISL.d = pdfFSIHSISL.dvalue()
            pdfFSIHSISL.DepartmentTotalPrint(pdfFSIHSISL.d)
            pdfFSIHSISL.ItemWiseClean()
            pdfFSIHSISL.ItemTypeClean()
            pdfFSIHSISL.DepartmentClean()
    ...
    pdfFSIHSISL.newrequest()
    pdfFSIHSISL.d = pdfFSIHSISL.newpage()

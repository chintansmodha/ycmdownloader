...
from PrintPDF import DGGainLoss_PrinrPDF as pdf
...
def DGGainLoss_PrintPDF(LSDepartmentCode, LCDepartmentCode, LDStartdate, LDEndDate):
    ...
    sql = "Select                  COALESCE(COSTCENTER.LONGDESCRIPTION,'') As Department " \
          ...
          ", Trim(Stxn.LOTCODE) As LotNo " \
          ", Stxn.TRANSACTIONNUMBER As AdjNo " \
          ", Stxn.TRANSACTIONDATE As Date " \
          ", Cast(Case When Stxn.Templatecode = '099' Then Stxn.USERPRIMARYQUANTITY Else 0 End As Decimal(15,3)) As Gain " \
          ", Cast(Case When Stxn.Templatecode = '098' Then Stxn.USERPRIMARYQUANTITY Else 0 End As Decimal(15,3)) As Loss " \
          ...
          "And   Stxn.TRANSACTIONDATE    Between   " + startdate + "    And     " + enddate + " " \
                                                                                              " " + Departmentcodes + " " \
                                                                                                                      "Order By Department, Item, LotNo, Date  "

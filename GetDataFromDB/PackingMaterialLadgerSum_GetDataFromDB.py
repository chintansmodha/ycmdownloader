...
from PrintPDF import PackingMaterialLadgerSum_PrintPDF as pdfrpt
...
def PackingMaterialLedgerSupplier_PrintPDF(LSCompany, LSParty, LSItem, LSPalleteType,LSReportType, LCCompany,LCParty, LCItem,LCPalleteType,LDStartDate, LDEndDate,request):
    ...
    sql = "SELECT PLANT.LONGDESCRIPTION AS PLANTNAME " \
          ",SUM(Case When  ID.TEMPLATECODE In ('PMS') And ID.PROVISIONALDOCUMENTDATE < "+startdate+" Then IDL.USERPRIMARYQUANTITY Else 0 End) as OPQTY " \
          ",0 as RECDQTY " \
          ",Sum(Case When  ID.TEMPLATECODE In ('PMS') And ID.PROVISIONALDOCUMENTDATE >="+startdate+"  Then IDL.USERPRIMARYQUANTITY Else 0 End) as ISSQTY " \
          ",SUM(Case When  ID.TEMPLATECODE In ('PMS') Then IDL.USERPRIMARYQUANTITY * Case When ID.PROVISIONALDOCUMENTDATE < "+startdate+"  " \
          "Then 1 Else -1 End Else 0 End) AS BALANCE " \
          ...
          "And ID.PROVISIONALDOCUMENTDATE <= "+enddate+" " \
          ...
          ", 0 As OPQTY " \
          ", SUM(PMD.PrimaryQuantity) As RECDQTY " \
          ", 0 As IssQty " \
          ", SUM(PMD.PrimaryQuantity) AS BALANCE " \
          ", BP.LEGALNAME1 AS SUPPLIER " \
          ", UGG.LongDescription AS PALLETENAME " \
          ...
          ", BP.LEGALNAME1 " \
          ", UGG.LongDescription " \
          "ORDER BY PLANTNAME, PALLETENAME, SUPPLIER "

...
from PrintPDF import PackingMaterialLedgerSumCustomer_PrintPDF as pdfrpt
...
def PackingMaterialLedgerCustomer_PrintPDF(LSCompany, LSParty, LSItem,LSPalleteType, LSReportType, LCCompany, LCParty, LCItem,LCPalleteType,LDStartDate, LDEndDate,request):
    ...
    sql="Select  PLANT.LONGDESCRIPTION AS PLANTNAME " \
        ...
        ", Sum(Case When  ID.TEMPLATECODE In ('PMC') And ID.PROVISIONALDOCUMENTDATE <"+startdate+" Then IDL.USERPRIMARYQUANTITY Else 0 End) As OPBal " \
        ",Sum(Case When  ID.TEMPLATECODE In ('PMC') And ID.PROVISIONALDOCUMENTDATE Between  "+startdate+" And "+enddate+" Then IDL.USERPRIMARYQUANTITY Else 0 End) as ReceivedQuantity " \
        ",0 As IssuedQuantity " \
        ",Sum(IDL.USERPRIMARYQUANTITY) AS BALANCE" \
        ...
        "Group by PLANT.LONGDESCRIPTION, BP.LEGALNAME1, UGG.LONGDESCRIPTION " \
        " Union All" \
        " Select  PKG.PLANTNAME,PKG.CUSTOMER " \
        ", PKG.PalletName " \
        ", 0 As OPBal " \
        ", 0 As ReceivedQuantity, Sum(PKG.PalletQuantity) As IssuedQuantity,-ABS(Sum(PKG.PalletQuantity)) AS BALANCE " \
        "From" \
        ...
        " Group By PKG.PLANTNAME,PKG.CUSTOMER, PKG.PalletName " \
        " Order By PLANTNAME, PalletName, CUSTOMER"
    ...
    if result == False:
        if counter > 0:
            ...
            pdfrpt.d = pdfrpt.dvalue(stdt, etdt, result, pdfrpt.plantcode)
            pdfrpt.PalletTotalPrint(pdfrpt.d)
            pdfrpt.d = pdfrpt.dvalue(stdt, etdt, result, pdfrpt.plantcode)
            pdfrpt.d = pdfrpt.dvalue(stdt, etdt, result, pdfrpt.plantcode)
            pdfrpt.DepTotalPrint(pdfrpt.d)
            pdfrpt.fonts(7)
            PMLV.Exceptions = ""
        elif counter == 0:
            PMLV.Exceptions = "Note: No Result found according to your selected criteria "

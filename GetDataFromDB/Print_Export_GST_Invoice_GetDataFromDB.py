...
from PrintPDF import PrintChallan_GST_Export_Invoice_PDF as pdfrptgstinvoice
...
LSInvoiceChallanNo=""
counter1=0
def PrintChallan_Export_GST_Invoice_PDF(request):
    global save_name
    LSName = datetime.now()
    LSstring = str(LSName)
    ...
    LSFileName = LSstring[0:4] + LSstring[5:7] + LSstring[8:10] + LSstring[11:13] + LSstring[14:16] + LSstring[
                                                                                                      17:19] + LSstring[
                                                                                                               20:]
    LSFileName = "PrintYeanChallan_Export_GST_INVOICE" + LSFileName
    ...
    PrintChallan_Export_GST_Invoice(LSInvoiceChallanNo,request,LSChallanNo)
...
def PrintChallan_Export_GST_Invoice(LSInvoiceChallanNo,request,LSChallanNo):
    print("from Export GST INvoice  challan")
    sql=""
    ...
    #       "            , PLANTINVOICE.CONTRACTNOCODE AS REFERANCECODE  " \
    #       "            , PlantInvoice.TIMEOFREMOVALOFGOODS AS SUPPLYDATE  " \
    #       "            , PlantInvoice.TIMEOFREMOVALOFGOODS AS DATEOFREMOVALOFGOODS " \
    ...
    #       "            , Coalesce(ADS_SaleLot.ValueString,'') AS LOTNUMBER   " \
    #       "            , Firm.TINNo AS CINNO " \
    ...
    #       "            , CAST(PLANTINVOICELINE.PRICE AS DECIMAL(18,2)) AS InvRate  " \
    #       "            , CAST((PLANTINVOICELINE.PRICE * PLANTINVOICELINE.PRIMARYQTY)AS DECIMAL (18,2)) AS ITEMAMOUNT   " \
    ...
    #       "           , Coalesce(LUT_Desc.ValueString,'') As LUT " \
    #       "            , count(*) over(partition by PLANTINVOICE.CODE)  as totalinvoiceitem  " \
    ...
    #       "    left JOIN AddressGst   CUSTOMERGSTIN  ON      CUSTOMERGSTIN.UniqueID = BusinessPartner.AbsUniqueId  " \
    ...
    #       "        Left Join       UserGenericGroup UGG_LUT        ON      UGG_LUT.USERGENERICGROUPTYPECODE = 'LUT' " \
    #       "        Left Join       ADStorage LUT_Desc              ON      UGG_LUT.ABSUNIQUEID = LUT_Desc.UniqueId " \
    #       "                                                AND     LUT_Desc.NameEntityName = 'UserGenericGroup' " \
    #       "                                                AND     LUT_Desc.FieldName = 'Description' " \
    #       "        Left Join       ADStorage LUT_StartDate         ON      UGG_LUT.AbsUniqueId = LUT_StartDate.UniqueId " \
    #       "                                                AND     LUT_StartDate.NameEntityName = 'UserGenericGroup' " \
    #       "                                                AND     LUT_StartDate.FieldName = 'StartingDate' " \
    #       "                                                AND     LUT_StartDate.ValueDate >= PLANTINVOICE.INVOICEDATE " \
    #       "        Left Join       ADStorage LUT_EndDate           ON      UGG_LUT.AbsUniqueId = LUT_EndDate.UniqueId " \
    #       "                                                AND     LUT_EndDate.NameEntityName = 'UserGenericGroup' " \
    #       "                                                AND     LUT_EndDate.FieldName = 'EndDate' " \
    #       "                                                AND     LUT_EndDate.ValueDate <= PLANTINVOICE.INVOICEDATE  " \
    #       "        Left Join       ADStorage LUT_Plant             ON      UGG_LUT.AbsUniqueId = LUT_Desc.UniqueId " \
    #       "                                                AND     LUT_Desc.NameEntityName = 'UserGenericGroup' " \
    #       "                                                AND     LUT_Desc.FieldName = 'PlantCodeCode' " \
    #       "                                                AND     LogicalWarehouse.PlantCode = LUT_Plant.ValueString " \
    #       "WHERE  " + LSInvoiceChallanNo +" "
    # sql=sql+"GROUP BY REGD_ADDRESS.addressline1   , REGD_ADDRESS.addressline2    , REGD_ADDRESS.addressline3    " \
    ...
    #         " , PLANTINVOICELINE.TARIFFCODE , PLANTINVOICE.TRUCKNO    " \
    #         "     , PLANTINVOICE.NETTVALUE   , PLANTINVOICE.IRN  " \
    #         "    , PLANTINVOICE.ROUNDOFFVALUE , LUT_Desc.ValueString " \
    ...
    sql = " SELECT     Coalesce(REGD_ADDRESS.addressline1,'') " \
          ...
          "            , Coalesce(PlantInvoice.TIMEOFREMOVALOFGOODS,'') AS DATEOFREMOVALOFGOODS " \
          ...
          "            , SALESDOCUMENTLINE.USERPRIMARYQUANTITY AS CHALLANQUANTITY   " \
          "            , SALESDOCUMENTLINE.USERPRIMARYUOMCODE AS CHALLANUOM  " \
          "            , Coalesce(BP_Trpt.Legalname1,'') AS TRANSPORTERNAME  " \
          "           , TermsOfShipping.LONGDESCRIPTION AS SUPPLYMODE  " \
          "           , DIVISION.code as DIVISIONCODE   " \
          "           , COMPANYBANK.CURRENTACCOUNTID AS ACCOUNTNUMBER   " \
          "           , BANK.BIC AS BANKIFSCCODE  " \
          ...
          "           , BANK.BANKBRANCHADDRESS AS BRANCHNAME" \
          "           , Coalesce(LUT_Desc.ValueString,'') As LUT " \
          "           , Coalesce(plantinvoice.EXPORTERREFNO,'')  AS EXPORTERREFERANCENO " \
          "           , Coalesce(plantinvoice.BUYERSPOREFNO,'') AS BUYERSPOREFNO " \
          "           , Coalesce(plantinvoice.ORDERCURRENCYCODE,'') as ORDERCURRENCYCODE  " \
          "           , Coalesce(plantinvoice.EXCHANGERATEOFCONTRACT ,'') as EXCHANGERATEOFCONTRACT  " \
          "    From PlantInvoice   " \
          "    JOIN SALESDOCUMENT      ON PLANTINVOICE.CODE                            = SALESDOCUMENT.PROVISIONALCODE  " \
          "                            and SALESDOCUMENT.DocumentTypeType = '06'  " \
          "    JOIN DIVISION                           ON PLANTINVOICE.DIVISIONCODE    = DIVISION.CODE  " \
          ...
          "    left JOIN Transportzone TZ_DespTo       on      ADDRESS_Consignee.TRANSPORTZONECODE     = TZ_DespTo.code " \
          "    left JOIN Transportzone TZ_DespFrom     on      Plant.TRANSPORTZONECODE     			   = TZ_DespFrom.code  " \
          "    left JOIN Transportzone BP_DespTo       on      ADDRESS_Consignee.TRANSPORTZONECODE     = BP_DespTo.code " \
          "    JOIN FIRM                               ON      plantinvoice.DIVISIONCODE = FIRM.CODE    " \
          ...
          "                                                AND     LUT_Desc.FieldName = 'PlantCodeCode' " \
          "                                                AND     LogicalWarehouse.PlantCode = LUT_Plant.ValueString " \
          "WHERE  SALESORDERIE.TYPEOFINVOICE='2'  and " + LSInvoiceChallanNo + " ORDER BY PLANTINVOICE.CODE"
    ...
    if result != False:
        ...
        while result != False:
            ...
            EXPORTERREFERANCENO=result['EXPORTERREFERANCENO']
            BUYERSPOREFNO=result['BUYERSPOREFNO']
            result = con.db.fetch_both(stmt)
        pdfrptgstinvoice.printlasttotal()
        pdfrptgstinvoice.printinvoicevalueinwords()
        pdfrptgstinvoice.getandprintFOBdeatils(pdfrptgstinvoice.invoiceno[-1])
        pdfrptgstinvoice.getandprinttaxdeatils(pdfrptgstinvoice.invoiceno[-1],pdfrptgstinvoice.roundoff[-1],EXPORTERREFERANCENO,BUYERSPOREFNO)
        if itype != '2':

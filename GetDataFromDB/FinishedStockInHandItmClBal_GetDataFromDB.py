...
def FinishedStockInHand_PrintPDF(LSDepartmentCode, LCDepartmentCode, LSQuality, LCQuality, LSWinding,
    ...
    sql = "Select          COALESCE(COSTCENTER.LONGDESCRIPTION,'') As Department " \
          ...
          ", COALESCE(UGG.LONGDESCRIPTION,'') As ShadeName , COALESCE(BKLELEMENTS.SALELOT,'') As SaleLot " \
          ...
          ", CAST((SUM (Case When ELEMENTS.ENTRYDATE <= " + startdate + " AND (Desp.TRANSACTIONDATE Is Null Or Desp.TRANSACTIONDATE > " + startdate + ") Then (BKLELEMENTS.ACTUALNETWT) Else 0 End)) As DECIMAL(18,3)) As NetWt " \
          ", CAST((SUM (Case When ELEMENTS.ENTRYDATE <= " + startdate + " AND (Desp.TRANSACTIONDATE Is Null Or Desp.TRANSACTIONDATE > " + startdate + ") Then (BKLELEMENTS.TOTALBOXES) Else 0 End)) As INT) As Boxes " \
          "From BKLELEMENTS " \
          ...
          "Group BY COALESCE(COSTCENTER.LONGDESCRIPTION,'')   , ITEMTYPE.CODE , BKLELEMENTS.LOTCODE " \
          ", PRODUCT.LONGDESCRIPTION ||' '|| COALESCE(QualityLevel.ShortDescription, '') " \
          ", COALESCE(UGG.LONGDESCRIPTION,'') , COALESCE(BKLELEMENTS.SALELOT,'') " \
          "Having CAST((SUM (Case When ELEMENTS.ENTRYDATE <= " + startdate + " AND (Desp.TRANSACTIONDATE Is Null Or Desp.TRANSACTIONDATE > " + startdate + ") Then (BKLELEMENTS.TOTALBOXES) Else 0 End)) As INT) <> 0 " \
    ...
    while result != False:
        ...
        pdfFSIHICB.textsize(pdfFSIHICB.c, result, pdfFSIHICB.d,stdt)
        pdfFSIHICB.d=pdfFSIHICB.dvalue()
        result = con.db.fetch_both(stmt)

        if pdfFSIHICB.d<40 :
            pdfFSIHICB.d=745
            pdfFSIHICB.c.showPage()
            pdfFSIHICB.header(stdt,pdfFSIHICB.divisioncode)
            pdfFSIHICB.fonts(7)
    ...
    if result == False:
        if counter > 0:
            ...
            pdfFSIHICB.d = pdfFSIHICB.dvalue()
            pdfFSIHICB.ItemwisePrint(pdfFSIHICB.d)
            pdfFSIHICB.d = pdfFSIHICB.dvalue()
            pdfFSIHICB.d = pdfFSIHICB.dvalue()
            pdfFSIHICB.ItemTypePrint(pdfFSIHICB.d)
            pdfFSIHICB.d = pdfFSIHICB.dvalue()
            pdfFSIHICB.d = pdfFSIHICB.dvalue()
            pdfFSIHICB.DepartmentPrint(pdfFSIHICB.d)
            pdfFSIHICB.ItemWiseClean()
            pdfFSIHICB.ItemTypeClean()
            pdfFSIHICB.DepartmentClean()
    ...
    pdfFSIHICB.newrequest()
    pdfFSIHICB.d = pdfFSIHICB.newpage()
    pdfFSIHICB.ItemWiseClean()
    pdfFSIHICB.ItemTypeClean()
    pdfFSIHICB.DepartmentClean()

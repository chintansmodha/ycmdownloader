...
from PrintPDF import FinishedStockInHandShadeBox_PrintPDF as pdf
...
def FinishedStockInHand_PrintPDF(LSDepartmentCode, LCDepartmentCode, LSQuality, LCQuality, LSWinding,
    ...
    sql = "Select          Comp.LONGDESCRIPTION as Plant " \
          ...
          "Order By Plant, Department, Product, ShadeName, BoxDt Desc, BoxNo  "
    ...
    if result == False:
        if counter > 0:
            pdf.d = pdf.dvalue(pdf.c, result, stdt)
            if pdf.shdeboxcount > 1:
                pdf.PrintShadeTotal()
                pdf.d = pdf.dvalue(pdf.c, result, stdt)
                pdf.d = pdf.dvalue(pdf.c, result, stdt)
            pdf.shdeboxcount = 0
            if pdf.itemshdecount > 1:
                pdf.PrintItemTotal()
                pdf.d = pdf.dvalue(pdf.c, result, stdt)
                pdf.d = pdf.dvalue(pdf.c, result, stdt)
            pdf.itemshdecount = 0

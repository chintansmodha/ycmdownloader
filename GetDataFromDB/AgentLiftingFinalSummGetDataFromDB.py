...
from PrintPDF import AgentLiftingFinalSumm_PrintPDF as pdfAGL
...
def AgentLifting_PrintPDF(LSBrokerGrp, LCBrokerGrp, LSBroker, LCBroker, LSPlant, LCPlant, LSParty, LCParty,
    ...
    sql = "Select          Plant.LONGDESCRIPTION AS Plant " \
          ...
          ", SUM(CAST(Pil.PRIMARYQTY As Decimal(20,3))) As Quantity " \
          ", SUM(CAST(Coalesce(Amount.Value, Pil.BASICVALUE, 0) AS DECIMAL(20,3))) As Amount " \
          ...
          ", BP.LEGALNAME1 " \
          ", Coalesce(Agent.LONGDESCRIPTION, 'Broker Name Not Entered') " \
          ", Coalesce(AgGrp.LONGDESCRIPTION, 'BrokerGroup Name Not Entered') " \
          ", Product.LONGDESCRIPTION ||' '|| Coalesce(UGG.LONGDESCRIPTION, '') ||' '|| Coalesce(QUALITYLEVEL.LONGDESCRIPTION,'')  " \
          "Order BY Plant, Agentgrp, Agent, Party, Item"
    ...
    if result == False:
        if counter > 0:
            ...
            pdfAGL.c.drawString(300, pdfAGL.d, "Party Total: ")
            pdfAGL.PartyTotalPrint()
            pdfAGL.d = pdfAGL.dvalues(pdfAGL.c, stdt, etdt, pdfAGL.divisioncode)
            pdfAGL.d = pdfAGL.dvalues(pdfAGL.c, stdt, etdt, pdfAGL.divisioncode)

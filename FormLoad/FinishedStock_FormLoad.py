...
",0 as OrdPenQty "\
 ...
 "Having  CAST((SUM (Case When ELEMENTS.ENTRYDATE <= '"+startdate+"' AND (Desp.TRANSACTIONDATE Is Null Or Desp.TRANSACTIONDATE > '"+startdate+"') Then (BKLELEMENTS.ACTUALNETWT) Else 0 End)) As DECIMAL(18,3)) <> 0.000  "\
          ...
          " And ID.PROVISIONALDOCUMENTDATE <= '"+startdate+"'" \
...
",0 as OrdPenQty "\
 ...
 "Having  CAST((SUM (Case When ELEMENTS.ENTRYDATE <= '"+startdate+"' AND (Desp.TRANSACTIONDATE Is Null Or Desp.TRANSACTIONDATE > '"+startdate+"') Then (BKLELEMENTS.ACTUALNETWT) Else 0 End)) As DECIMAL(18,3)) <> 0.000  "\
    ...
    return render(request, 'FinishedStock.html',{'GDataItemTypeCode':GDataItemTypeCode,"GDataQualityLevelCode":
        GDataQualityLevelCode,"GDataPlantCode":GDataPlantCode,"GDataDetail":GDataDetail,"GDataPlantDetail":GDataPlantDetail
        ,"plantclqtytotal":round(plantclqtytotal),"plantordpendqtytotal":round(plantordpendqtytotal),'plantopnqtytotal':round(plantopnqtytotal)
        ,'plantissqtytotal':round(plantissqtytotal),'plantisscopstotal':round(plantisscopstotal)
        ,'plantrecqtytotal':round(plantrecqtytotal),'plantclcopstotal':round(plantclcopstotal),'plantclboxestotal':round(plantclboxestotal)
        ,"itemclqtytotal":round(itemclqtytotal),"itemorderpendqtytotal":round(itemorderpendqtytotal),"itemclcopstotal":round(itemclcopstotal)
        ,"itemclboxestotal":round(itemclboxestotal),'itemrecqtytotal':round(itemrecqtytotal),'itemissqtytotal':round(itemissqtytotal)
        ,'itemisscopstotal':round(itemisscopstotal),'itemopenqtytotal':round(itemopenqtytotal),"startdate":startdate})

...
def PurchaseItemWiseDetailHtml(request):
    ...
    while result1 != False:
        ...
        result1 = con.db.fetch_both(stmt1)
    return render(request,'PurchaseItemWiseDetail.html', {'GDataItemCode':GDataItemCode,'GDataCompanyCode':GDataCompanyCode,'GDataQuality':GDataQuality})

def PurchaseItemWiseSummaryHtml(request):
    global GDataItemCode
    global GDataQuality
    global GDataCompanyCode
    ...
    stmt1 = con.db.exec_immediate(con.conn,"select UNIQUEID, VALUESTRING from ADStorage where FIELDNAME='AutogeneratedName' order by VALUESTRING")
    result1 = con.db.fetch_both(stmt1)
    while result1 != False:
        if result1 not in GDataProductionItemGroup:
            GDataProductionItemGroup.append(result1)
        result1 = con.db.fetch_both(stmt1)
    return render(request,'PurchaseItemWiseSummary.html', {'GDataItemCode':GDataItemCode,'GDataCompanyCode':GDataCompanyCode,'GDataQuality':GDataQuality,"GDataProductionItemGroup":GDataProductionItemGroup})

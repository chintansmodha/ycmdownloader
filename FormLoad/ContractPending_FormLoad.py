...
def ContractPending_ItemGroup(request):
      ...
      sql = "Select " \
            ...
            ", sum(cast(Case When SO.DOCUMENTTYPETYPE In ('00','02') Then SOD.UserPrimaryQuantity - SOD.UsedUserPrimaryQuantity Else 0 End as decimal(18)))  As PendContractQty " \
            ", sum(cast(Case When SO.DOCUMENTTYPETYPE = '03' Then SOL.UserPrimaryQuantity Else 0 End as decimal(18)))  As DespOrderQty" \
            ", sum(cast(Case When SO.DOCUMENTTYPETYPE = '03' Then SOD.UserPrimaryQuantity - SOD.UsedUserPrimaryQuantity Else 0 End as decimal(18)))  As PendDespOrderQty " \
            ", sum(cast(Case When SO.DOCUMENTTYPETYPE In ('00','02') Then SOL.UserPrimaryQuantity Else 0 End as decimal(18)))  As ContractQty " \
            " from SalesOrder SO" \
...
def ContractPending_ItemDetail(request):
      ...
      sql = "Select  Product.LONGDESCRIPTION as Item" \
            ...
            " where SOL.PROGRESSSTATUS in(0,1) " \
            " and SO.DOCUMENTTYPETYPE in ('00','02','03')" \
            " and Product.ItemTypeCode='"+LSItemCode+"'" \
      ...
      while result != False:
            result['TOTAL'] = round(float(result['PENDCONTRACTQTY']) + float(result['PENDDESPORDERQTY']))
            detcontractqty = round(detcontractqty + float(result['CONTRACTQTY']))
            detdespnotgn = round(detdespnotgn + float(result['PENDCONTRACTQTY']))
            detgoodnotgn = round(detgoodnotgn + float(result['PENDDESPORDERQTY']))
            dettotal = round(dettotal + float(result['TOTAL']))
            GDataItemDetail.append(result)
            result = con.db.fetch_both(stmt)
      return render(request, 'ContractPending.html',
                    {'GDataItemGroup': GDataItemGroup,'GDataItemDetail': GDataItemDetail,'GDataItemType':GDataItemType,
                     ...
                     ,'detcontractqty': round(detcontractqty, 2), 'detdespnotgn': round(detdespnotgn, 2)
                          , 'detgoodnotgn': round(detgoodnotgn, 2), 'dettotal': round(dettotal, 2),'itemgroup':LSItemCode,"ItemType":ItemType})

def ContractPending_ItemSubDetail(request):
      ...
      for i in range(0, len(itemtype) + 1, 4):
          if i + 3 < len(itemtype) + 1:
              ...
              b = '' + a + ''
              ItemType.append(b)
      sql="Select cast(sum(Case When SO.DOCUMENTTYPETYPE In ('00','02') Then SOD.UserPrimaryQuantity - SOD.UsedUserPrimaryQuantity Else 0 End+Case When SO.DOCUMENTTYPETYPE = '03' Then SOL.UserPrimaryQuantity Else 0 End) as decimal(18)) as baldesp," \
          ...
          ", cast(Case When SO.DOCUMENTTYPETYPE In ('00','02') Then SOL.UserPrimaryQuantity Else 0 End as decimal(18)) As ContractQty" \
          ", cast(Case When SO.DOCUMENTTYPETYPE In ('00','02') Then SOD.UserPrimaryQuantity - SOD.UsedUserPrimaryQuantity Else 0 End as decimal(18)) As PendContractQty" \
          ", cast(Case When SO.DOCUMENTTYPETYPE = '03' Then SOL.UserPrimaryQuantity Else 0 End as decimal(18)) As DespOrderQty" \
          ", cast(Case When SO.DOCUMENTTYPETYPE = '03' Then SOD.UserPrimaryQuantity - SOD.UsedUserPrimaryQuantity Else 0 End as decimal(18)) As PendDespOrderQty" \
          ...
          " where SOL.PROGRESSSTATUS in(0,1)" \
          " and SO.DOCUMENTTYPETYPE in ('00','02','03')" \
          " and Product.LongDescription='"+LSItem+"'" \
      ...
      return render(request, 'ContractPending.html',
                    {'GDataItemSubDetail':GDataItemSubDetail,'GDataItemGroup': GDataItemGroup, 'GDataItemDetail': GDataItemDetail,
                     ...
                     'sumcontractqty': round(sumcontractqty, 2), 'sumdespnotgn': round(sumdespnotgn, 2)
                          , 'sumgoodnotgn': round(sumgoodnotgn, 2), 'sumtotal': round(sumtotal, 2)
                          , 'detcontractqty': round(detcontractqty, 2), 'detdespnotgn': round(detdespnotgn, 2)
                          , 'detgoodnotgn': round(detgoodnotgn, 2), 'dettotal': round(dettotal, 2),'item':item,'itemgroup':LSItemCode,"ItemType":ItemType})
...
def ContractPending_ApplyType(request):
    ...
    return render(request, 'ContractPending.html', {'GDataItemGroup': GDataItemGroup, 'GDataItemType': GDataItemType,
        ...
        , 'sumgoodnotgn': round(sumgoodnotgn, 2), 'sumtotal': round(sumtotal, 2),"ItemType":itemtype})
